...
- [Reports](#reports)
  ...
  - [flow.json and flow-summary.md](#flowjson-and-flow-summarymd)
...
| `flows` | `[FlowContract]` | No | Ordered screenshot journeys that should be validated as coherent flows |
...
| `flowUnknownStep` | `.error` | A flow references a screen ID not declared in `screens` |
| `flowMissingRequiredStep` | `.error` | A required flow step has no screenshot evidence |
| `flowDuplicateStep` | `.warning` | A flow declares the same screen ID more than once |
...
### flow.json and flow-summary.md

Flow reports explain whether a screenshot folder contains the ordered screen
states declared by the contract. `flow.json` is machine-readable; `flow-summary.md`
is meant for design and QA review.
...
Flow step status is one of:

| Status | Meaning |
|---|---|
| `present` | Screen is declared and screenshot evidence exists |
| `missing` | Screen is declared but the screenshot is missing |
| `unknown` | Flow references a screen ID not declared in `screens` |
